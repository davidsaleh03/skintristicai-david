...
const Race: React.FC<AgeProps> = ({ allData }) => {
  ...
  const selectedRace = useSelector(
    (state: RootState) => state.race.selectedRace,
  );
  const dispatch = useDispatch();

  const percentage = selectedRace
    ? Math.round(Number(allData.data.race[selectedRace]) * 100)
    : 0;
...
const strokeDashoffset = circumference - (percentage / 100) * circumference;
  ...
  useEffect(() => {
    if (allData?.data?.race && !selectedRace) {
      const sorted = Object.entries(allData.data.race).sort(
